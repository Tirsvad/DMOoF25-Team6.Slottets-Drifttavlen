...
- The solution is managed via `DMOoF25-Team6.Slottets-Drifttavlen.slnx` and `global.json`.

## Architecture & Patterns
- **Clean Architecture**: Core business logic is in `DMOoF25-Team6.Slottets-Drifttavlen.Core`, domain models in `DMOoF25-Team6.Slottets-Drifttavlen.Domain`, and infrastructure (e.g., data access) in `DMOoF25-Team6.Slottets-Drifttavlen.Infrastructure`.
- **Testing**: Test projects mirror the main projects (e.g., `DMOoF25-Team6.Slottets-Drifttavlen.Core.Tests` for Core). Place new tests in the corresponding `tests/` subfolder.
...
## Developer Workflows
- **Build**: Run `dotnet build DMOoF25-Team6.Slottets-Drifttavlen.slnx` from the repo root.
- **Test**: Run `dotnet test DMOoF25-Team6.Slottets-Drifttavlen.slnx` to execute all tests.
...
## Examples
- To add a new domain entity: place it in `src/DMOoF25-Team6.Slottets-Drifttavlen.Domain/` and add related logic in `Core` and `Infrastructure` as needed.
